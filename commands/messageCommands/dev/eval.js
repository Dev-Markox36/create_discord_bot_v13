...
module.exports = {
    ...
    usage: "c",
    description: "asfg",
    run: async (client, msg, args, user, text, prefix, MessageEmbed, userModel) => {
    
        if (!args.length) return msg.channel.send('Pon algo para evaluar.');
        let code = args.join(" ");
        const words = ['client.destroy()', 'client.destroy', 'client.token', 'process.env', 'process.env.TOKEN', 'token', 'process.env.MONGO_ACCOUNT', 'MONGO_ACCOUNT', 'mongoose'];
        ...
        async function sendMsg(msg, res) {
            try {
                if (res.length < 2000) {
